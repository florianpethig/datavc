...
df_vercon1 <- df_results %>%
  select(vercon1) %>%
  mutate(vercon1 = strsplit(gsub(" ", "", vercon1), ",")) %>%
  unnest(vercon1) %>% count(vercon1) %>% slice(1:3) %>% rename(vercon =
                                                                 vercon1)
df_vercon2 <- df_vercon %>%
  select(vercon2) %>%
  mutate(vercon2 = strsplit(gsub(" ", "", vercon2), ",")) %>%
  unnest(vercon2) %>% count(vercon2) %>% slice(1:3) %>% rename(vercon =
                                                                 vercon2)
df_vercon3 <- df_vercon %>%
  select(vercon3) %>%
  mutate(vercon3 = strsplit(gsub(" ", "", vercon3), ",")) %>%
  unnest(vercon3) %>% count(vercon3) %>% slice(1:3) %>% rename(vercon =
                                                                 vercon3)

type <-
  c(
    rep("Manuscript \n (e.g., Word, LaTeX)", 3),
    rep("Data analysis \n (e.g., py script, DO-file)", 3),
    rep("Dataset \n (e.g., csv, dta, xls)", 3)
  )
...
labels <-
  c(vercon = "Custom Value 1", "2" = "Custom Value 2", "3" = "Custom Value 3")

theme_set(theme_classic())
p <- ggplot(df_combined, aes(fill = vercon, y = n, x = ordered)) +
  geom_bar(position = "fill", stat = "identity") +
  theme(axis.title.x = element_blank(), axis.title.y = element_blank()) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(fill = "Type of version control") +
